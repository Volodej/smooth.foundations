...
{
    // TODO: test in Unity for MainThread synchronization 
    ...
    public static class ResultAsyncExtensions
    {
        ...
        public static Task<ResultEx<TResult>> ThenTryAsync<TValue, TResult>(this Result<TValue> result, Func<TValue, Task<TResult>> func)
        {
            ...
            try
            {
                return func(result.Value).ContinueWith(task => ResultEx<TResult>.FromValue(task.Result), CancellationToken.None,
                    TaskContinuationOptions.DenyChildAttach | TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
            }
            catch (Exception e)
            {
            ...
            Func<Exception, string> catchFunc)
        {
            ...
            try
            {
                return func(result.Value).ContinueWith(task => Result.FromValue(task.Result), CancellationToken.None,
                    TaskContinuationOptions.DenyChildAttach | TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
            }
            catch (Exception e)
            {
            ...
            return resultTask.ContinueWith((task, f) =>
                {
                    ...
                    return ((Func<T, bool>) f)(result.Value)
                        ? result
        ...
        public static Task ForEach<T>(this Task<Result<T>> resultTask, Action<T> action, TaskScheduler scheduler = null)
        {
            return resultTask.ContinueWith((task, a) => task.Result.ForEach((Action<T>) a),
