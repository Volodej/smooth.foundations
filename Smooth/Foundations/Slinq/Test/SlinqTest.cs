...
namespace Smooth.Slinq.Test {
	...
	public class SlinqTest : MonoBehaviour {
		...
		//public static readonly DelegateFunc<ValueTuple<int, int>, ValueTuple<int, int>, bool> eq_1_p = Comparisons<ValueTuple<int, int>>.ToPredicate(eq_1);
		...
		private void TestCorrectness() {
			...
			if (!tuples1.Slinq().GroupJoin(tuples2.Slinq(), to_1, to_1, (a, bs) => (a.Item1, a.Item2, bs.Count()))
			    .SequenceEqual(tuples1.GroupJoin(tuples2, to_1f, to_1f, (a, bs) => (a.Item1, a.Item2, bs.Count())).Slinq(), eq_t3)) {
				Debug.LogError("GroupJoin failed.");
				testCorrectness = false;
			}
			
			if (!tuples1.Slinq().Join(tuples2.Slinq(), to_1, to_1, (a, b) => (a.Item1, a.Item2, b.Item2))
			    .SequenceEqual(tuples1.Join(tuples2, to_1f, to_1f, (a, b) => (a.Item1, a.Item2, b.Item2)).Slinq(), eq_t3)) {
			...
			if (!tuples1.Slinq().Zip(tuples2.Slinq()).SequenceEqual(
				...
				.Select(x => (tuples1[x], tuples2[x])))) {
				Debug.LogError("Zip tuples failed.");
				testCorrectness = false;
			}

			if (!tuples1.Slinq().Zip(tuples2.Slinq(), (a, b) => (a.Item1, b.Item1)).SequenceEqual(
				Slinqable.Sequence(0, 1)
				.TakeWhile(x => x < tuples1.Count && x < tuples2.Count)
				.Select(x => (tuples1[x].Item1, tuples2[x].Item1)), eq)) {
			...
			if (!tuples1.Slinq()
			    .ZipAll(tuples2.Slinq().Skip(midSkip), (a, b) => (a, b))
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x + midSkip < tuples2.Count)
			               .Select(x => (
				x < tuples1.Count ? new Option<ValueTuple<int, int>>(tuples1[x]) : new Option<ValueTuple<int, int>>(),
