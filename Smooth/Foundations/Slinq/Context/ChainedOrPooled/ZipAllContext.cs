...
namespace Smooth.Slinq.Context
{
    ...
    public struct ZipAllContext<T2, C2, T, C>
    {
        ...
        public static Slinq<ValueTuple<Option<T>, Option<T2>>, ZipAllContext<T2, C2, T, C>> ZipAll(Slinq<T, C> left, Slinq<T2, C2> right,
...
#pragma warning restore 0414
        ...
        private static void Skip(ref ZipAllContext<T2, C2, T, C> context, out Option<ValueTuple<Option<T>, Option<T2>>> next)
        {
            ...
            if (context.needsMove)
            {
                if (context.left.current.isSome)
                {
                    ...
                    if (context.right.current.isSome) context.right.skip(ref context.right.context, out context.right.current);
                }
                else
                {
        ...
        private static void Dispose(ref ZipAllContext<T2, C2, T, C> context, out Option<ValueTuple<Option<T>, Option<T2>>> next)
        {
            ...
            if (context.left.current.isSome)
            {
                ...
                if (context.right.current.isSome) context.right.dispose(ref context.right.context, out context.right.current);
            }
            else
            {
    ...
    public struct ZipAllContext<U, T2, C2, T, C>
    {
        ...
        public static Slinq<U, ZipAllContext<U, T2, C2, T, C>> ZipAll(Slinq<T, C> left, Slinq<T2, C2> right,
            Func<Option<T>, Option<T2>, U> selector, ZipRemoveFlags removeFlags)
        {
...
#pragma warning restore 0414
    ...
    public struct ZipAllContext<U, T2, C2, T, C, P>
    {
        ...
        public static Slinq<U, ZipAllContext<U, T2, C2, T, C, P>> ZipAll(Slinq<T, C> left, Slinq<T2, C2> right,
            Func<Option<T>, Option<T2>, P, U> selector, P parameter, ZipRemoveFlags removeFlags)
        {
...
#pragma warning restore 0414
        ...
        private ZipAllContext(Slinq<T, C> left, Slinq<T2, C2> right, Func<Option<T>, Option<T2>, P, U> selector, P parameter,
            ZipRemoveFlags removeFlags)
        {
            needsMove = false;
            this.left = left;
