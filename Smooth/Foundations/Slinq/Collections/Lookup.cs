	...
	public class Lookup<K, T> : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            if (dictionary.Count > 0)
            {
                ...
                foreach (var values in dictionary.Values) valuesAcc.Append(values);
                dictionary.Clear();
            }
        ...
        public static Lookup<K, T> Borrow(IEqualityComparer<K> comparer)
        {
            lock (pool)
            {
                return pool.Count > 0 ? pool.Pop() : new Lookup<K, T>(comparer);
            }
        }
        ...
        public void Add(K key, T value)
        {
            ...
            if (!dictionary.TryGetValue(key, out values)) keys.Append(key);
            values.Append(value);
        ...
        {
            LinkedHeadTail<T> values;
            if (dictionary.TryGetValue(key, out values)) dictionary.Remove(key);
            return values;
        }
        ...
        ///     Ownership of the lookup and any values it contains is retained by the caller.
        /// </summary>
        public Slinq<U, GroupJoinContext<U, K, T, T2, C2, P>> GroupJoinAndKeep<U, T2, C2, P>(Slinq<T2, C2> outer,
        ...
        /// </summary>
        public Slinq<U, GroupJoinContext<U, K, T, T2, C2>> GroupJoinAndDispose<U, T2, C2>(Slinq<T2, C2> outer, Func<T2, K> outerSelector,
            Func<T2, Slinq<T, LinkedContext<T>>, U> resultSelector)
        {
        ...
        ///     Ownership of the lookup and any values it contains is retained by the caller.
        /// </summary>
        public Slinq<U, JoinContext<U, K, T, T2, C2, P>> JoinAndKeep<U, T2, C2, P>(Slinq<T2, C2> outer, Func<T2, P, K> outerSelector,
        ...
        ///     When the enumeration is complete, the lookup and any values it contains are added to the disposal queue.
        /// </summary>
        public Slinq<U, JoinContext<U, K, T, T2, C2>> JoinAndDispose<U, T2, C2>(Slinq<T2, C2> outer, Func<T2, K> outerSelector,
            Func<T2, T, U> resultSelector)
        {
