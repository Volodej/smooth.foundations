...
namespace Smooth.Slinq.Collections
{
	...
	public class Linked<T> : IDisposable
    {
        ...
        ///     Note: Nodes should generally not be released one at a time as it will incur much greater overhead than batching releases using
        ///     LinkedHeadTail<T>s.
        /// </summary>
	...
	public class Linked<K, T> : IDisposable
    {
        ...
        /// <summary>
        ///     Traverses the node list that starts with this node, clears the key and value of each node, and releases the resulting node list to the
        ///     pool.
        /// </summary>
        public void Dispose()
        {
	...
	public struct LinkedHeadTail<T> : IEquatable<LinkedHeadTail<T>>
    {
        ...
        public void DisposeInBackground()
        {
            if (head != null) head.DisposeInBackground();
            head = null;
            tail = null;
            count = 0;
        }
	...
	public static class Linked
    {
	    ...
	    ///     The specified list must be well formed when calling this method or the program will enter an invalid state, resulting in unspecified
	    ///     behaviour.
	    ///     Calling this method will invalidate the specified list and any variables containing its nodes.
	    /// </summary>
	    ...
	    public static LinkedHeadTail<T> Sort<T>(LinkedHeadTail<T> input, Comparison<T> comparison, bool ascending)
        {
            ...
            if (input.count == 2)
            {
                if ((ascending ? comparison(input.head.value, input.tail.value) : comparison(input.tail.value, input.head.value)) <=
                    0) return input;
            ...
            for (var i = 1; i < left.count; ++i) left.tail = left.tail.next;
	    ...
	    /// <summary>
	    ///     Merges the specified sorted lists using the specified comparison and ordering.  Elements from the left list will appear before elements
	    ///     from the right on equal comparisons.
	    ///     The specified lists must be well formed when calling this method or the program will enter an invalid state, resulting in unspecified
	    ///     behaviour.
	    ...
	    public static LinkedHeadTail<T> Merge<T>(LinkedHeadTail<T> left, LinkedHeadTail<T> right, Comparison<T> comparison, bool ascending)
        {
            ...
            if (ordering * comparison(left.head.value, right.tail.value) > 0)
            {
	    ...
	    ///     Calling this method will invalidate the specified lists and any variables containing their nodes.
	    /// </summary>
	    public static LinkedHeadTail<K, T> Merge<K, T>(LinkedHeadTail<K, T> left, LinkedHeadTail<K, T> right, Comparison<K> comparison,
            bool ascending)
        {
            if (left.count == 0)
                return right;
            ...
            if (ordering * comparison(left.head.key, right.tail.key) > 0)
            {
                right.tail.next = left.head;
                right.tail = left.tail;
                right.count += left.count;
	    ...
	    public static LinkedHeadTail<T> InsertionSort<T>(LinkedHeadTail<T> input, Comparison<T> comparison, bool ascending)
        {
            ...
            while (input.head != null)
            {
                ...
                else
                {
                    ...
                    while (after.next != null && ordering * comparison(current, after.next.value) >= 0) after = after.next;
	    ...
	    public static LinkedHeadTail<K, T> InsertionSort<K, T>(LinkedHeadTail<K, T> input, Comparison<K> comparison, bool ascending)
        {
            ...
            while (input.head != null)
            {
                ...
                {
                    var after = sorted.head;
                    while (after.next != null && ordering * comparison(current, after.next.key) >= 0) after = after.next;
                    insert.next = after.next;
                    after.next = insert;
                }
            }
