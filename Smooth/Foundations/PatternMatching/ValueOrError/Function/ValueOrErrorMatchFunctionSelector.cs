...
namespace Smooth.Foundations.PatternMatching.ValueOrError.Function
{
    public sealed class ValueOrErrorMatchFunctionSelector<T, TResult>
    {
        ...
        private readonly List<ValueTuple<Func<T, bool>, Func<T, TResult>>> _predicatesAndFuncs;
        private readonly bool _isError;

        public ValueOrErrorMatchFunctionSelector(Func<T, TResult> defaultFunction, bool isError)
        {
            _defaultFunction = defaultFunction;
            _predicatesAndFuncs = new List<ValueTuple<Func<T, bool>, Func<T, TResult>>>();
            _isError = isError;
        }

        public void AddPredicateAndAction(Func<T, bool> test, Func<T, TResult> action) =>
            _predicatesAndFuncs.Add(new ValueTuple<Func<T, bool>, Func<T, TResult>>(test, action));
