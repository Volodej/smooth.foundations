    ...
    public static class Factory
    {
        ...
        public static Option<IEqualityComparer<T>> EqualityComparer<T>()
        {
            ...
            return expression.isSome
                ? new Option<IEqualityComparer<T>>(
                    new FuncEqualityComparer<T>(Expression.Lambda<Func<T, T, bool>>(expression.value, l, r).Compile()))
        ...
        private static IComparer<T> KeyValuePairComparer<T>(Type type)
        {
            ...
            var keysCompared = Expression.Lambda<Comparison<T>>(Expression.Call(keyComparer.Item1, keyComparer.Item2, keyL, keyR), l, r)
                .Compile();
            var valuesCompared = Expression
                .Lambda<Comparison<T>>(Expression.Call(valueComparer.Item1, valueComparer.Item2, valueL, valueR), l, r).Compile();
        ...
        /// <summary>
        ///     Returns an option containing an expression that compares l and r for equality without casting, or None if no such comparison can be
        ///     found.
        /// </summary>
        public static Option<Expression> EqualsExpression(Expression l, Expression r)
        {
            try
            {
                ...
                if (method != null)
                {
                    ...
                    if (ps[0].ParameterType == l.Type && ps[1].ParameterType == r.Type) return new Option<Expression>(expression);
                }
            }
            ...
            try
            {
                ...
                if (mi != null && mi.GetParameters()[0].ParameterType == r.Type) return new Option<Expression>(Expression.Call(l, mi, r));
            }
            catch (Exception e)
            {
                SmoothLogger.LogError(e);
        ...
        public static ValueTuple<Expression, MethodInfo> ExistingComparer(Type type)
        {
            ...
            return new ValueTuple<Expression, MethodInfo>(
                c,
                c.Type.GetMethod("Compare", BindingFlags.Public | BindingFlags.Instance, null, new[] {type, type}, null));
        }
        ...
        public static ValueTuple<Expression, MethodInfo, MethodInfo> ExistingEqualityComparer(Type type)
        {
            ...
            return new ValueTuple<Expression, MethodInfo, MethodInfo>(
                ec,
                ec.Type.GetMethod("Equals", BindingFlags.Public | BindingFlags.Instance, null, new[] {type, type}, null),
                ec.Type.GetMethod("GetHashCode", BindingFlags.Public | BindingFlags.Instance, null, new[] {type}, null));
        }
